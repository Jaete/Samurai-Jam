class_name BossStateMachine extends Node2D
...
func change_state(source_state: BossState, next_state_name: StringName):
	changing_state = true
...
#		return
	var next_state: BossState = states.get(next_state_name.to_lower())
	if !next_state:
